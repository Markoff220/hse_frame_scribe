...
Видео → .md нейроконспект в Obsidian: ffmpeg → Silero VAD → GigaAM ASR → кадры + Ollama VLM → map-reduce конспект (Ollama LLM).
Git-репозитория нет. Папка лежит внутри Obsidian-vault, результаты пишутся прямо в vault.
...
- Корень: докер-файлы (`Dockerfile`, `docker-compose.yml`, `.env`, `.dockerignore`), `config.json` / `config.docker.json`, `requirements.txt`, `.bat`-скрипты (Windows).
- `config.json` — локальный конфиг; `config.docker.json` — для контейнера (выбирается переменной `VIDEONOTES_CONFIG`). Переопределения через env: `OLLAMA_URL`, `VLM_MODEL`, `LLM_MODEL`, `ASR_DEVICE`.
...
- `output.dir` разрешается относительно корня проекта (не CWD): в `config.json` это `../../03_База/Видео-конспекты`, т.е. внутрь vault. Не переносить папку проекта без правки конфига.
- `in/` — watch-папка (`processed/`, `failed/`); `tmp/` — scratch (uploads, `jobs.json`, можно чистить); `logs/` — `job_<id>.log`, `web_*.log`, `run_*.log`; `models/gigaam` — веса ASR.

## Команды
- Самопроверка: `python app/pipeline/main.py selftest` (ffmpeg, GigaAM — при первом запуске скачает ~1 ГБ с CDN Sber, кадры, Ollama).
- Одно видео: `python app/pipeline/main.py process <video>`; слежение за `in/`: `watch`.
...
## Подводные камни
- Код в `app/`, а конфиги и данные (`config*.json`, `in/`, `tmp/`, `logs/`, `models/`) — в корне. `ROOT` вычисляется в `app/pipeline/common.py` поиском вверх по дереву папки с `config.json`/`config.docker.json`: локально — корень проекта, в контейнере — `/app` (там смонтирован `config.docker.json`). Все пути — от него, не от `app/`.
...
- Состояние задач персистится в `tmp/jobs.json` (после рестарта сервиса «running» → «error»); upload > 8 ГБ отклоняется (413).
